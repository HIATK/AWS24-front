...
const Search = () => {
  ...
  const fetchResults = async (searchTerm: string, page: number) => {
    setLoading(true);
    try {
      const response = await fetch(`http://ec2-43-201-54-252.ap-northeast-2.compute.amazonaws.com:5000/api/movies/search?keyword=${encodeURIComponent(searchTerm)}&page=${page}`);
      if (response.ok) {
