...
const Search = () => {
  ...
  const fetchResults = async (searchTerm: string, page: number) => {
    setLoading(true);
    try {
      const response = await fetch(`http://ec2-3-35-122-73.ap-northeast-2.compute.amazonaws.com:8000/api/movies/search?keyword=${encodeURIComponent(searchTerm)}&page=${page}`);
      if (response.ok) {
  ...
  useEffect(() => {
    ...
    const expectedURL = `http://ec2-3-35-122-73.ap-northeast-2.compute.amazonaws.com:3000/movies/search?keyword=${encodeURIComponent(searchTerm)}`;
